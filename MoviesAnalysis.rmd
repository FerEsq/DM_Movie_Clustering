...
# Seed for the random sample
set.seed(1313)
kmeans_model <- kmeans(movies_new, centers = 3, nstart = 25)
```
```{r}
# Ignore outliers
fviz_cluster(
...
```{r}
silhouette_kmeans <- silhouette(kmeans_model$cluster, dist(movies_new))
```
```{r}
silhouette_hierarchical <- silhouette(cutree(hirarchical_model, k = 3), dist(movies_new))
```
```{r}
fviz_silhouette(silhouette_kmeans) + labs(title = "Silhouette Analysis - K-means")
```
```{r}
fviz_silhouette(silhouette_hierarchical) + labs(title = "Silhouette Analysis - Hierarchical")
```
...
Se han analizado 3 y 4 clusters como se indicaba en el ejercicio anterior. Podemos observar que el clustering jerárquico tiene un desempeño superior al clustering de K-means. Con K=4 el resultado es engañoso, pues el promedio es tan alto debido a que ha agrupado prácticamente todos los datos en el segundo cluster. En conclusión, el mejor desempeño se obtiene con el clustering jerárquico con 3 clusters.